...
function detalle(target) {
    ...
    window.location.replace("?p=detalleGenero&id=" + id);
}
/**
...
function eliminar(target) {
    var id = $(target).data("id");
    httpConnect("/genero/" + id, null, "DELETE",
            function (r) {
                alert(r.message);
                cargarDatos();
            });
...
function cargarDatos() {
    httpConnect("/genero", null, "GET", function (r) {
        ...
        for (var i = 0; i < r.data.length; i++) {
            var entity = r.data[i];
            html += "<tr>";
            html += "<td>" + entity.genNombre + "</td>";
            html += "<td>";
            html += "<div data-id='" + entity.genId + "' class='material-icons delete' style='color:red'>delete</div>";
            html += "<div data-id='" + entity.genId + "' class='material-icons edit' style='color:green'>edit</div>";
            html += "</td>";
            html += "</tr>";
        }
        $("tbody").html(html);
